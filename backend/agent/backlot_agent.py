...
The synthetic incident telemetry is stored in datasource uid='{prom_uid}' (Prometheus)
and uid='{loki_uid}' (Loki).

The incident time window is: {q_start} to {q_end}

Investigation steps:
1. Call query_prometheus with:
